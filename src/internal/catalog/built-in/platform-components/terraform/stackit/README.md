...
```bash
cd platform-configs/terraform/<CLUSTERNAME>/bootstrap-tfstate-backend
```
...
- `platform-configs/terraform/<cluster-name>/infrastructure/env.auto.tfvars`
...
1. Configure `edge_instance` in `platform-configs/terraform/<cluster-name>/infrastructure/env.auto.tfvars`.
